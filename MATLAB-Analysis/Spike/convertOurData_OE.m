%% Adds dummy channels to single channel recordings for OPEN-EPHYS DATA
% Specify the filename in namePart1 and numFiles
% Specify data channel(s) & PD/ADC channel(s)

clear pathToDataFolder
...
dataFolderName = 'Wing_ephys_181028\';
...
namePart1 = dataFolderName; % dataFolderName = dataFolderName(dataFolderName ~= '-');
namePart1b = '_converted';
namePart2 = '.bin';
    % DATA FILE
newFileName = [ namePart1, namePart1b, namePart2 ];
newFileName = newFileName(newFileName ~= '\'); newFileName(newFileName ~= '/');
    % ADC FILE
newFileNameADC = [ namePart1,'_ADC', namePart1b, namePart2 ];
...
for ii=1:length(adcChan)
    [tempADC(ii,:), ~, ~] = load_open_ephys_data([pathToDataFolder,dataFolderName,'100_ADC',num2str(dataChan(ii)),'.continuous']);
    disp(['finished loading ADC : ', num2str(dataChan(ii))]);
end 
...
temp = int16(temp);
dataRAW(1:size(temp,1),:) = invertBit*temp(:,:);
...
adcRAW = ones(initVar2, length(tempADC),'int16');
