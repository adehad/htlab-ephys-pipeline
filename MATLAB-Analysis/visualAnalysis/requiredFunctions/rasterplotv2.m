...
%   RASTERPLOT(T,L) Plots the rasters of spiketimes (T in samples) each of length
%   L samples, Sampling rate = 1kHz. Spiketimes are hashed by the trial length.
%
%   RASTERPLOT(T,L,H) Plots the rasters in the axis handle H
%
%   RASTERPLOT(T,L,H,FS) Plots the rasters in the axis handle H. Uses sampling rate of FS (Hz)
%
%   Example:
...
% D.Ko, Inspired by Rajiv Narayan
% dsk13@ic.ac.uk
% Imperial College London

function rasterplotv2(timeCell,triallen, varargin)
...
%%%%%%%%% Code Begins %%%%%%%%%%%%
switch nin
    case 2 %no handle so plot in a separate figure
        figure;
        hresp=gca;
        fs=defaultfs;
    case 3 %handle supplied
        hresp=varargin{1};
        if (~ishandle(hresp))
...
% plot spikes
times = [];
numtrials = length(timeCell);
for ii = 1:numtrials
    temp = timeCell{ii} + triallen*(ii-1);
    times = [times; temp];
end
